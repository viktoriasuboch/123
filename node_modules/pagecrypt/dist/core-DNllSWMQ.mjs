...
async function loadCrypto() {
	var _globalThis, _window;
	if ((_globalThis = globalThis) === null || _globalThis === void 0 ? void 0 : _globalThis.crypto) return globalThis.crypto;
	else if ((_window = window) === null || _window === void 0 ? void 0 : _window.crypto) return window.crypto;
	else return (await import("node:crypto")).webcrypto;
}
const crypto = await loadCrypto();

//#endregion
//#region src/decrypt-template.html
...
async function getEncryptedPayload(content, password, iterations) {
	if (iterations < 2e6) console.warn(`[pagecrypt] WARNING: The specified number of password iterations (${iterations}) is not secure. If possible, use at least 2_000_000 or more.`);
...
async function encryptHTML(inputHTML, password, iterations = 2e6) {
	return decrypt_template_default.replace("<encrypted-payload></encrypted-payload>", `<pre class="hidden" data-i="${iterations.toExponential()}">${await getEncryptedPayload(inputHTML, password, iterations)}</pre>`);
}
/**
...
function generatePassword(length = 80, characters = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz") {
	if (characters.length > 255) throw new Error("[pagecrypt] Max character set length is 255");
...
//#endregion
export { generatePassword as n, encryptHTML as t };